...
(function () {
  ...
  /**
   * Menyimpan item apa adanya, termasuk id-nya. Dipakai HANYA oleh impor
   * cadangan — di alur biasa selalu pakai tambahItem() supaya id, createdAt,
   * dan nilai bawaan terisi dengan benar.
   */
  async function simpanItemUtuh(item) {
    if (!item || !item.id) throw new Error('Item tidak punya id, tidak bisa disimpan.');
    try {
      var store = await ambilStore('readwrite');
      await bungkus(store.put(item), 'Gagal menyimpan item dari cadangan.');
      return item;
    } catch (e) {
      throw pesanManusiawi(e, 'Gagal menyimpan item dari cadangan.');
    }
  }

  /** Mengosongkan seluruh data. Dipakai oleh "Ganti semua" dan Hapus semua data. */
  async function hapusSemuaItem() {
    try {
      var store = await ambilStore('readwrite');
      await bungkus(store.clear(), 'Gagal mengosongkan data.');
      return true;
    } catch (e) {
      throw pesanManusiawi(e, 'Gagal mengosongkan data.');
    }
  }
  ...
  window.DB = {
    ...
    hapusItem: hapusItem,
    simpanItemUtuh: simpanItemUtuh,
    hapusSemuaItem: hapusSemuaItem,
